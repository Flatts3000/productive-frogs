...
- **In-game guide: shipped on `main` (#336, closes #318).** The 2.0 guide is built on **Modonomicon** (`26.1.2-neoforge:2.1.0`, soft-dep, book id `productivefrogs:guide`) - 10 chapters / 40 entries / 76 pages, with the four boss-altar 3D diagrams generated from the GameTest-locked structure NBTs (Modonomicon has no NBT import). Engine, scope, chapter map, and voice live in `docs/guidebook.md`; `GuidebookTest` guards entry-id/geometry/lang, and the copy is authored in the mc-pack-toolkit quest-voice Guides register. Unreleased on `main` (ships in the next release).
