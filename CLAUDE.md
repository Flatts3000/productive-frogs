...
### The Equivalence (EE) lane (#253) - opt-in, default OFF

A post-capstone, RF-powered transmutation lane for **off-roster** items (items with no `slime_variant`, i.e. `findByPrimer == null`): Alembic (item + bucket + RF -> Mimic Slime Bucket) -> Milker -> Mimic Milk -> Mimic Slimes -> Midas frog (Kiss-primed) -> Prismatic Froglight -> Distiller (+ RF) -> the original item. Design doc: `docs/equivalence_lane.md`. The defining constraint was **"break nothing"** - it's built as standalone registrations plus isolated additive branches in shared code, walled off so the six-species machinery is untouched. Non-obvious pieces:

- **`SYNTHESIZED_ITEM` is the lane's join key** - an arbitrary item id data component (mutually exclusive with `SLIME_VARIANT`). The Mimic Slime, its bucket, Mimic Milk, and the Prismatic Froglight all carry it instead of a variant id.
- **`MimicSlime` is a sibling of vanilla `Slime`, NOT a `ResourceSlime` subclass.** That's deliberate: `ResourceFrogAttackablesSensor` and `FrogTongueDropHandler` gate the species path on `instanceof ResourceSlime`, so a Mimic Slime is invisible to the six species by construction. Only the Midas branch (in the same sensor) + `MidasTongueDropHandler` eat it. It's always size 1, never splits.
- **Mimic Milk is ONE source-only fluid** (`MimicMilkFluid.Source/Flowing` both `canSpreadTo -> false`); the synthesized item rides the `MimicMilkSourceBlockEntity`, not the fluid identity (sidesteps the v1.8 per-variant-fluid wall). Documented caveat: a pipe that places the *fluid block* directly bypasses the bucket, so the source won't know its item - PF's own machines read the BE. The `MimicMilkFluidBucketWrapper` preserves the item across a bucket<->tank round-trip.
- **`client/SynthesizedTint.colorFor(Item)`** is the runtime color resolver (sprite-average of opaque pixels, block map-color fallback, no-cache-on-failure). Client-only (reads the atlas) - server particle code uses a fixed color instead. Cache is cleared on resource reload.
- **Alembic + Distiller are PF's first RF machines** - same Block + BE + Menu + Screen furnace shape as the appliances, but with a receive-only `EnergyStorage(cap, maxReceive, 0)` subclass exposing an internal `consume(int)`, registered as `Capabilities.EnergyStorage.BLOCK` (the **1.21.1** id). The Alembic's `canSynthesize` is the dupe/launder guard (rejects bucket/container/**any component-bearing**/pipeline/on-roster items); both machines use an airtight insert-output-then-consume-inputs transaction.
- **Midas is a `midas` flag on `ResourceFrog`, NOT a 7th `Category`** (a 7th category would spray phantom entries through every `Category.values()` loop). It has its own `MIDAS_FROG_EGG` block + spawn eggs, breeds true (`canMate` Midas-only), and the flag is **synced on both the frog AND the tadpole** (`DATA_MIDAS`) so names read correctly client-side. The marker threads egg BE -> tadpole -> frog.
- **The whole lane is gated by `PFConfig.equivalenceEnabled()`** (fail-closed, default OFF): when off, recipes drop, creative/JEI entries hide, and every behavioural surface no-ops (machine ticks, the Kiss -> Midas-egg prime, the Midas drop, Mimic source + Sprinkler spawns). Dev runs force it on via `-Dproductivefrogs.equivalence=true` (build.gradle); GameTests that drive the machines set the `PFConfig.equivalenceEnabledOverride` test hook.
