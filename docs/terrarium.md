...
- **Redstone control (per Sprinkler):** a Sprinkler **pauses spawning while it receives a redstone signal** (hopper convention - powered = OFF). It is just another "pause without spending" branch in `SprinklerBlockEntity.serverTick` (`hasNeighborSignal`), so the held milk, catalysts, and spawn budget freeze and resume the instant the signal drops. This lets a comparator (e.g. a storage-full read) throttle resource production on a per-Sprinkler basis. No config flag - it is inert unless you wire redstone to the ceiling cell (powered from above the roof is the natural hookup).
