...
- **Real (non-placeholder) textures** for every item/block. Status by surface:
  ...
  - ⏳ **Slime Milker block faces** (top/side/bottom + a front) — still solid gray placeholder. Needs custom pixel art (Tier B, AI-gen + touchup).
  - ⏳ **Slime Milker GUI** (`textures/gui/container/slime_milker.png`) — minimal gray rect; needs themed background with progress arrow + player inventory split.
  - ⏳ **Parent slime entities** (Cave / Geode / Tide / Void) — partial body textures, need bespoke pixel art (Tier C).
  - ⏳ **`tadpole_silhouette.png` / `slime_silhouette.png`** — blank/transparent, need art.
