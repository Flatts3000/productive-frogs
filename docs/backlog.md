...
- **J4** ✅ — Source-block slime spawning. `SlimeMilkSourceBlock extends LiquidBlock` overrides `onPlace` + `tick` to drive a self-scheduling block-tick loop independent of the fluid's flow-tick. Success cadence uniform [200, 600] ticks (10–30s); blocked-slot retry [40, 80] ticks (2–4s). Spawn-position picker prefers the top of a solid horizontal neighbour (rim) and falls back to the column above. Variant mapping: `"vanilla"` → vanilla Slime, `"magma"` → vanilla MagmaCube, other 12 → `ResourceSlime` with the matching SlimeVariant. Entity overlap is allowed; only motion-blocking terrain skips a candidate position. Covered by 5 GameTests (iron rim spawn, copper column-fallback, vanilla-slime mapping, magma-cube mapping, fully-blocked skip).
