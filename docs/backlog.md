...
- **Debug observability framework (all layers).** Specced in [observability.md](./observability.md). One cross-cutting `PFDebug` helper (per-layer areas, gated by `-Dproductivefrogs.debug=<areas>` system property + a `/pf debug <area> on|off` command), off by default, that logs each layer's resolution decisions to `latest.log` with a greppable `[PF/<area>]` prefix. Replaces the add-then-delete ad-hoc logging. Phased build: Phase 1 = core + `render`/`tint` areas (the client-render diagnosis need); Phase 2 = gameplay handlers; Phase 3 = blocks/registries/lifecycle. **Lesson that motivated it:** client-render bugs (tints, inner-block resolution, render types) are invisible to GameTest; the 2026-05-25 inner-block investigation burned ~25 round-trips re-reading correct code while the running build diverged, and a one-line render-thread log ended it in one launch. When a visual bug can't be reproduced by reading correct code, suspect a stale dev build first.
