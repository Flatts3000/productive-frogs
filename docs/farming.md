...
- **When ON**: each milk source block carries a `spawnsRemaining` counter on its BlockEntity that starts at `depletionCount` on placement and decrements by one per spawn. When it reaches zero the next tick replaces the block with air (true drain - not a fluid-state reset). (Pre-v1.7 this was a blockstate property capped at 16; it moved to the BE so Count catalysts can raise it without bound.)
...
- **Catalysts** (v1.7): drop **Count / Speed / Quantity / Infinite** catalysts into a placed source to raise its remaining count, spawn faster, spawn more per event, or never deplete. The buffs save to the source and survive re-bucketing. Full spec: [slime_milk_catalysts.md](./slime_milk_catalysts.md).
- **Look-at readout**: Jade shows the spawns-remaining count (or "unlimited") plus any catalyst Speed/Quantity levels. A standalone in-world visual countdown remains deferred to the polish backlog.
...
| Burn the Froglight as fuel (Coal / Blaze / Lava, v1.8.3 + v1.19.0) | Powers a furnace | Coal Froglight burns like a coal item (1600 ticks, 8 smelts); Blaze Froglight like a blaze rod (2400 ticks, 12 smelts); Lava Froglight like a lava bucket (20000 ticks, 100 smelts, v1.19.0). Every other Froglight is inert (not fuel). |

**Smelting**: works on every variant Froglight. Each variant has its own smelt recipe → its base resource.

**Fuel (v1.8.3, v1.19.0)**: the Coal, Blaze, and Lava Froglights double as furnace fuel, burning for the same time as the vanilla item each is made of - a coal item (1600 ticks), a blaze rod (2400 ticks), and a lava bucket (20000 ticks, added v1.19.0) respectively. The Lava Froglight makes the renewable-lava loop double as a renewable solid fuel with no empty bucket to manage. This is per-stack behaviour keyed off the Froglight's variant component (every Froglight is the same `configurable_froglight` item), so only those variants ignite; all others remain decorative. Implemented as a `getBurnTime` override on `ConfigurableFroglightItem` rather than the `furnace_fuels` data map, which keys on the item and would otherwise make every variant fuel.
