...
Status of this doc: **implemented + verified on `feat/spawnery`** - build + 60 GameTests green, and the manual `runClient` visual pass passed 2026-05-26 (GUI + tooltips, per-variant milk in JEI, the frogspawn-teal window idle/lit, disabled-state hiding all confirmed in-client). Release-ready.
...
- `blockstates/spawnery.json`: `facing` (n/e/s/w) x `lit`; `lit=false` -> `spawnery`, `lit=true` -> `spawnery_on`, y-rotated per facing.
- `models/block/spawnery.json` + `spawnery_on.json`: `minecraft:block/orientable_with_bottom` over the bespoke `spawnery_*` textures (`_on` swaps top/front/side to the lit variants; bottom is shared).
- `models/item/spawnery.json`: `{"parent":"productivefrogs:block/spawnery"}`.
- **Textures (done):** bespoke `spawnery_{top,front,side,bottom}` + `spawnery_{top,front,side}_on`, generated by `scripts/generate_spawnery_textures.ps1` - derived from the Slime Milker (same cobblestone + wood-plank build, framed window) with the window recoloured to a frogspawn teal: a dark egg pool (stippled) when idle, a bright teal glow when lit, where the Milker shows green slime. Same appliance family, visually distinct. Confirmed in-client 2026-05-26.
