...
- **Textures (done):** bespoke `spawnery_{top,front,side,bottom}` + `spawnery_{top,front,side}_on`, generated by `scripts/generate_spawnery_textures.ps1` - derived from the Slime Milker (same cobblestone + wood-plank build, framed window) with the window recoloured to a frogspawn teal: a dark egg pool (stippled) when idle, a bright teal glow when lit, where the Milker shows green slime. Same appliance family, visually distinct. Still owed: a `runClient` look to confirm it reads right in-world.
