...
| `[frog_stats]` | `enabled` | `true` | Master switch for the Appetite / Bounty / Reach stat-breeding layer. `false` gives the plain six-species loop with no breeding minigame: Sweetslime is uncraftable + hidden and no longer breeds frogs, every frog behaves at the baseline (no per-frog variance), and the Jade stat lines are hidden. Stored stats on existing frogs are frozen (not deleted) - re-enabling restores them. Recipe gating needs a world reload. |
...
| `[variants]` | `disabledIntegrations` | `[]` | Host mods whose cross-mod variants you want force-off, by modid (e.g. `["mekanism", "powah"]`). Every variant gated solely behind a listed mod is disabled, like `disabledVariants`. Lets you keep a host mod for its machines while dropping its Resource Slime variants. |
| `[boss]` | `enabled` | `true` | Master switch for the boss tier (2.0: the four boss ALTARS - Wither, Dragon, Warden, Elder Guardian). `false` drops the altar block recipes, hides them from JEI + creative, and suppresses any pack-added weight-0 prime-only variant. (The 1.x boss variants, catalyst altars, and `bossVariantsEnabled` retired in the 2.0 mob-drop redesign.) Recipe gating needs a world reload. |

### Scoping content with `[variants]`

Use this to ship a smaller mod surface without datapack surgery - say, only the Cave and Bog lines (`disabledCategories = ["geode", "tide", "infernal", "void"]`), to drop a single noisy resource (`disabledVariants = ["productivefrogs:redstone"]`), or to keep a host mod for its machines while dropping its frog variants (`disabledIntegrations = ["mekanism"]`). The toggles are a **soft hide**: the registry entry stays, so a disabled variant's already-placed slimes, buckets, and froglights keep working, and re-enabling restores everything (no save surgery). One caveat - a disabled variant may still have its per-variant Slime Milk *fluid* registered (that is minted at mod-init, before this config loads), but it stays unobtainable since every way to reach the variant is gated. Changes apply on world reload.

**`disabledIntegrations` specifics.** The provider mod is read from each variant's `mod_loaded` gate, so the key is that gate's modid. Most cross-mod metals come through **AllTheOres** (tin, lead, osmium, nickel, silver, zinc, aluminum, uranium are all under `"alltheores"`, not a per-metal mod); the rest are under their obvious host (`mekanism`, `powah`, `create`, `ae2`, `refinedstorage`, `mythicmetals`, `mysticalagriculture`, `industrialforegoing`, `fluxnetworks`, `justdirethings`). A variant shared by several providers (only `silicon`, gated `ae2 OR refinedstorage`) is disabled **only when every** provider is listed - disabling one leaves it reachable via the other; to drop such a variant outright, name it in `disabledVariants`. Listing a modid that isn't installed is a harmless no-op (its variants already don't load). Only built-in variants are covered; a variant added through a live world datapack carries no provider mapping.
