...
`scripts/generate_slime_milk_textures.ps1` now overlays two dark pixel dots at (6,3) and (9,3) on the milk surface after the tint pass — eye positions chosen against the vanilla `milk_bucket.png` milk region (y=3 is the top of the widest milk band; x=6 and x=9 give vanilla-slime-style 2-pixel eye spacing). Eye color is fixed (28,28,28), so the multiplication-tint pipeline keeps them dark on every variant. Held / dropped slime milk buckets now read as "slime peering out of an iron bucket" instead of "tinted dye."
