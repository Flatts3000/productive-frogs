...
### 🟡 Iron Furnaces refused to accept Froglights at all until a restart (patched here, open upstream)
The single most-reported Sky Frogs problem: no Froglight would enter any Iron Furnace, by hand or by pipe, while a vanilla furnace took them fine, and restarting the game fixed it for a while. `hasRecipe` memoises smeltability in `ModSetup.HAS_RECIPE`, a **static `Map<Item, Boolean>`** living for the whole process, so the first stack of an item ever tested answers for every later one. Froglight recipes match on the `slime_variant` component, so a **variant-less** Froglight (creative tab, or one whose component a Copy-Paste Gadget dropped - see the Building Gadgets entry) genuinely has no recipe, caches `false` against the Froglight item, and locks out every variant until the process ends. Same root cause as the auto-split bug below, different method. Fixed in **Productive Frogs 1.25.4**: component-carrying stacks bypass the cache, everything else keeps the original path. `compat.ironFurnacesRecipeCacheFix`. See **[ironfurnaces_component_fixes.md](https://github.com/Flatts3000/productive-frogs/blob/mc-1.21.1/docs/ironfurnaces_component_fixes.md)**.

### 🟡 Iron Furnaces auto-split converts Froglight variants (patched here, open upstream)
A factory-augmented Iron Furnace with **auto-split** on pools every input slot holding "the same item" and averages the counts, deciding "the same item" on the item id alone. Every Froglight variant is one id plus a `slime_variant` component, so a furnace holding 64 of one variant and 1 of another comes out holding 32 and 33 - a silent, unattended, 1:1 converter from the cheapest Froglight to the most expensive. Any single-id-plus-component item from any mod is corrupted the same way. Stays listed here rather than archived: the upstream defect is open, and our workaround carries a standing obligation to re-verify on every Iron Furnaces bump.

Upstream is [Qelifern/IronFurnaces#229](https://github.com/Qelifern/IronFurnaces/issues/229), a 1.21 data-component regression of their own [#147](https://github.com/Qelifern/IronFurnaces/issues/147) (fixed for 1.19.2 NBT in 2023). It is open, unanswered, and the repository's last commit predates the report, so we patch it: **[ironfurnaces_component_fixes.md](./ironfurnaces_component_fixes.md)**. On by default, `compat.ironFurnacesAutoSplitFix`, no effect without the mod. **Re-verify on any Iron Furnaces bump** - the injector fails open on purpose, so a broken patch is silent.
