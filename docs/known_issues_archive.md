...
### 🟢 Chorus Fruit Froglight smelted into popped chorus fruit instead of chorus fruit - resolved (2026-06-05)
The `chorus_fruit` variant Froglight smelted to `minecraft:popped_chorus_fruit` - the last remaining variant that didn't smelt back to the resource it represents (its primer item). Like the clay-ball -> brick bug fixed on 2026-05-28, it applied vanilla's chorus-fruit -> popped furnace transformation instead of returning the chorus fruit the slime was primed with, which also broke the loop's self-sufficiency (raw chorus fruit is the primer, so a pack whose only end-resource source is the frog loop could never mint more chorus slimes). Players who want popped chorus fruit can still smelt the raw fruit themselves - the vanilla chain is one extra furnace step, not lost. Fix: changed the smelt result to `minecraft:chorus_fruit` in both the generator source of truth (`scripts/generate_v1_1_variants.ps1`, the `chorus_fruit` row's `Smelt`) and the generated recipe (renamed `configurable_froglight_chorus_fruit_to_popped_chorus_fruit.json` -> `configurable_froglight_chorus_fruit_to_chorus_fruit.json`, matching the `_to_<resource>` naming of the others). The "non-1:1 smelt" notes in `docs/backlog.md` and `docs/species_as_category_redesign.md` were updated in the same pass; `docs/v1_1_scope.md` keeps the original decision as a historical scope record.
