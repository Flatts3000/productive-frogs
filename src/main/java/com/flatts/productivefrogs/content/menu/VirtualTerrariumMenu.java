 ...
 * the upgrade column, the player inventory, and the cycle progress/interval/energy
 * data for the GUI meters. The feedstock and molten product live in FluidTanks on
 * the BlockEntity, driven through the client-synced BlockEntity + ContainerData.
 */
public class VirtualTerrariumMenu extends AbstractContainerMenu {
    ...
    public VirtualTerrariumMenu(int containerId, Inventory playerInv, VirtualTerrariumBlockEntity be, ContainerData data) {
        ...
        if (be != null) {
            ...
            for (int i = 0; i < VirtualTerrariumInventory.OUTPUT_COUNT; i++) {
                addSlot(new SlotItemHandler(inv, VirtualTerrariumInventory.OUTPUT_START + i, outputX(i), outputY(i)) {
                    @Override
            ...
            for (int i = 0; i < VirtualTerrariumInventory.UPGRADE_COUNT; i++) {
                addSlot(new Slot(dummy, VirtualTerrariumInventory.UPGRADE_START + i, UPGRADE_X, UPGRADE_START_Y + i * 18) {
                    @Override
                    public boolean mayPlace(ItemStack stack) {
                        return stack.is(PFItemTags.VIRTUAL_TERRARIUM_UPGRADE);
                    }
                });
            }
        }
    ...
    /** Buffered molten product in mB, for the output tank gauge. */
    public int getProductAmount() {
        return dataAccess.get(VirtualTerrariumBlockEntity.DATA_PRODUCT);
    }

    /** Stored RF, for the energy meter. */
    public int getEnergyStored() {
        return dataAccess.get(VirtualTerrariumBlockEntity.DATA_ENERGY);
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        ...
        if (VirtualTerrariumBlockEntity.isEmptyBucket(carried)) {
            swap = blockEntity.drainToBucket();
        } else if (VirtualTerrariumBlockEntity.isFeedstockBucket(carried)) {
            swap = blockEntity.fillFromBucket(carried);
        } else {
            return false;
        }
        if (swap.isEmpty()) {
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        ...
        if (slot.hasItem()) {
            ...
            } else if (stack.is(PFItemTags.VIRTUAL_TERRARIUM_UPGRADE)) {
