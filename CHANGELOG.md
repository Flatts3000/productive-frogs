...
- **The Froglight Cleaver.** A late-game sword that drops a Resource Slime's Froglight when it kills it - the active-play counterpart to the passive frog loop. Looting raises the yield, and a brewed slime's effect carries onto the Froglight, exactly like a frog eating it. Crafted from boss Froglights (Nether Star + Dragon Egg) and Dragon's Breath, so it is pure endgame. Config-gated (`froglight_weapon.enabled`, default on). (#212)
